	...
	struct willis_error_info* error)
{
	...
	if (error_xcb != NULL)
	{
		willis_error_throw(context, error, WILLIS_ERROR_WIN_XINPUT_GET_POINTER);
		return;
	}
	...
	if (error_xcb != NULL)
	{
		willis_error_throw(context, error, WILLIS_ERROR_WIN_XINPUT_SELECT_EVENTS);
		return;
	}
	...
	struct willis_error_info* error)
{
	...
	if (error_xcb != NULL)
	{
		willis_error_throw(context, error, WILLIS_ERROR_WIN_XKB_SELECT_EVENTS);
		return;
	}
	...
	struct willis_error_info* error)
{
	...
	struct xkb_keymap* keymap =
		xkb_win_keymap_new_from_device(
			xkb_common->context,
			backend->conn,
			backend->xkb_device_id,
	...
	if (keymap == NULL)
	{
		willis_error_throw(context, error, WILLIS_ERROR_WIN_XKB_KEYMAP_NEW);
		return;
	}

	struct xkb_state* state =
		xkb_win_state_new_from_device(
			keymap,
			backend->conn,
			backend->xkb_device_id);
	...
	if (state == NULL)
	{
		...
		willis_error_throw(context, error, WILLIS_ERROR_WIN_XKB_STATE_NEW);
		return;
	}
